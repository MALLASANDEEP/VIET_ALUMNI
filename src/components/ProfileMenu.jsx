...
import { User, LogOut, LayoutDashboard, Mail, Linkedin, GraduationCap } from "lucide-react";
import { Button } from "@/components/ui/button";
import { DropdownMenu, DropdownMenuContent, DropdownMenuItem, DropdownMenuSeparator, DropdownMenuTrigger, } from "@/components/ui/dropdown-menu";
import { Dialog, DialogContent } from "@/components/ui/dialog";
import { useNavigate } from "react-router-dom";
const ProfileMenu = ({ profile, onLogout }) => {
      ...
      <DropdownMenu>
        <DropdownMenuTrigger asChild>
          <Button variant="ghost" className="relative h-10 w-10 rounded-full border border-white/20 p-0 overflow-hidden hover:bg-white/10">
            {profile.photo_url ? (<img src={profile.photo_url} alt={profile.full_name} className="h-full w-full object-cover"/>) : (<User className="h-5 w-5 text-white"/>)}
          </Button>
        </DropdownMenuTrigger>
        ...
        <DialogContent className="max-w-[95vw] md:max-w-2xl p-0 overflow-hidden border-none shadow-2xl rounded-2xl">
          ...
          <div className="bg-gradient-to-br from-orange-500 to-amber-600 p-5 md:p-8 text-white">
            <div className="flex flex-col sm:flex-row sm:items-center gap-4 sm:gap-6">
              <div className="h-20 w-20 sm:h-24 sm:w-24 rounded-full bg-white/20 backdrop-blur-md flex items-center justify-center overflow-hidden border-4 border-white/50 shadow-xl">
                {profile.photo_url ? (<img src={profile.photo_url} alt={profile.full_name} className="h-full w-full object-cover"/>) : (<User className="h-12 w-12 text-white"/>)}
              </div>
              <div>
          ...
          <div className="p-5 md:p-8 space-y-8 max-h-[70vh] overflow-y-auto bg-white">
            ...
            <div className="pt-6 border-t border-gray-100 flex flex-col gap-6">
              {profile.linkedin_url && (<a href={profile.linkedin_url} target="_blank" rel="noopener noreferrer" className="inline-flex items-center gap-2 text-blue-600 font-medium hover:underline">
