...
import { useIncomingConnectionRequests, useOutgoingConnectionRequests, useRespondConnectionRequest, } from "@/hooks/useConnections";
const DashboardNotificationBell = ({ profile }) => {
    ...
    const previousOutgoingStatusRef = useRef({});
        ...
        for (const request of outgoing) {
            ...
            if (before === "pending" && (now === "accepted" || now === "rejected")) {
                toast({
                    ...
                    description: now === "accepted"
                        ? `${request.receiver?.full_name || "Alumni"} accepted your request.`
                        : `${request.receiver?.full_name || "Alumni"} rejected your request.`,
                });
            }
        }
        previousOutgoingStatusRef.current = Object.fromEntries(outgoing.map((request) => [request.id, request.status]));
    }, [outgoing]);
    return (<Popover>
      <PopoverTrigger asChild>
        <Button variant="ghost" size="icon" className="relative text-primary-foreground hover:text-gold">
          <Bell className="w-5 h-5"/>
          {badgeCount > 0 && (<span className="absolute -right-0.5 -top-0.5 min-w-5 h-5 px-1 rounded-full bg-destructive text-white text-[10px] font-semibold flex items-center justify-center">
        ...
        <ScrollArea className="h-[380px]">
          <div className="p-4 space-y-5">
            <section className="space-y-3">
              ...
              {incomingLoading ? (<div className="flex justify-center py-4">
                  <Loader2 className="w-5 h-5 animate-spin text-gold"/>
                </div>) : pendingIncoming.length === 0 ? (<p className="text-xs text-muted-foreground">No pending requests.</p>) : (pendingIncoming.map((request) => (<div key={request.id} className="rounded-lg border p-3 bg-muted/30">
                    <p className="text-sm font-medium">{request.sender?.full_name || "User"}</p>
                    <p className="text-[11px] text-muted-foreground mt-1">
                      wants to connect with you
                    </p>
                    <div className="flex gap-2 mt-3">
                      <Button size="sm" className="h-8 px-3 text-xs gap-1" disabled={respond.isPending} onClick={() => respond.mutate({
                ...
                status: "accepted",
                      ...
                      <Button size="sm" variant="outline" className="h-8 px-3 text-xs gap-1" disabled={respond.isPending} onClick={() => respond.mutate({
                requestId: request.id,
                senderProfileId: request.sender_profile_id,
                receiverProfileId: request.receiver_profile_id,
                status: "rejected",
            ...
            <section className="space-y-3">
              ...
              {outgoingLoading ? (<div className="flex justify-center py-4">
                  <Loader2 className="w-5 h-5 animate-spin text-gold"/>
                </div>) : responseUpdates.length === 0 ? (<p className="text-xs text-muted-foreground">No responses yet.</p>) : (responseUpdates.slice(0, 6).map((request) => (<div key={request.id} className="rounded-lg border p-3 bg-muted/20 flex items-center justify-between gap-3">
            ...
            <section className="space-y-3">
              ...
              {outgoingLoading ? (<div className="flex justify-center py-4">
                  <Loader2 className="w-5 h-5 animate-spin text-gold"/>
                </div>) : recentOutgoing.length === 0 ? (<p className="text-xs text-muted-foreground">No sent requests yet.</p>) : (recentOutgoing.map((request) => (<div key={request.id} className="rounded-lg border p-3 flex items-center justify-between gap-3">
                    <div>
                    ...
                    <Badge variant={request.status === "pending" ? "outline" : request.status === "accepted" ? "default" : "secondary"} className="capitalize">
                      {request.status}
                    </Badge>
                  </div>)))}
            </section>
