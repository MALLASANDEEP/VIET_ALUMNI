...
import { Briefcase, Users, MapPin, ExternalLink, Building, Clock, IndianRupee, Loader2, UserPlus, MessageCircle } from "lucide-react";
...
export const StudentDashboard = ({ profile, showNetworkTabs = true }) => {
    ...
    return (<Tabs defaultValue="jobs" className="space-y-6">
      <TabsList className="w-full justify-start overflow-x-auto whitespace-nowrap rounded-xl border border-border bg-card p-1 [scrollbar-width:none] [&::-webkit-scrollbar]:hidden">
        ...
        {showNetworkTabs && (<TabsTrigger value="connections" className="gap-2 px-3 text-xs sm:text-sm">
            <UserPlus className="w-4 h-4"/>
            Connections
          </TabsTrigger>)}
        {showNetworkTabs && (<TabsTrigger value="chat" className="gap-2 relative px-3 text-xs sm:text-sm">
        ...
        <Card>
          ...
          <CardContent>
            {jobsLoading ? (<div className="flex justify-center py-8">
              ...
              </div>) : jobs && jobs.length > 0 ? (<div className="grid gap-4">
                {jobs.map((job, index) => (<motion.div key={job.id} initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: index * 0.1 }} className="bg-muted/50 rounded-xl p-6 hover:bg-muted/70 transition-colors border border-transparent hover:border-gold/20">
                    <div className="flex flex-col md:flex-row md:items-start md:justify-between gap-6">
                      <div className="flex-1 space-y-3">
                        <div>
                          ...
                          <div className="flex flex-wrap items-center gap-x-4 gap-y-1 mt-1 text-sm">
                            ...
                            {job.location && (<div className="flex items-center gap-1.5 text-muted-foreground">
                                <MapPin className="w-4 h-4"/>
                                {job.location}
                              </div>)}
                            {job.salary_range && (<div className="flex items-center gap-1.5 text-muted-foreground">
                        ...
                        {job.skills_required && job.skills_required.length > 0 && (<div className="flex flex-wrap gap-1">
                            {job.skills_required.map((skill) => (<span key={skill} className="text-[10px] bg-background px-2 py-0.5 rounded border border-border text-muted-foreground">
                                {skill}
                              </span>))}
                          </div>)}

                        <div className="flex items-center justify-between pt-2">
                          {job.alumni && (<div className="flex items-center gap-2">
                              <div className="w-6 h-6 rounded-full bg-gold/10 flex items-center justify-center overflow-hidden">
                                {job.alumni.photo_url ? (<img src={job.alumni.photo_url} className="w-full h-full object-cover"/>) : (<span className="text-[10px] text-gold font-bold">{job.alumni.full_name.charAt(0)}</span>)}
                              </div>
                              <span className="text-xs text-muted-foreground">
                                Posted by {job.alumni.full_name}
                              </span>
                            </div>)}
                          {job.application_deadline && (<div className="flex items-center gap-1 text-[11px] text-destructive font-medium">
                      ...
                      <div className="flex flex-row md:flex-col gap-2 shrink-0">
                        <Dialog>
                          ...
                          <DialogContent className="max-w-2xl max-h-[90vh] overflow-y-auto">
                            ...
                            <div className="space-y-6 py-4">
                              ...
                              {job.responsibilities && (<div className="space-y-2">
                              ...
                              {job.eligibility && (<div className="space-y-2">
                              ...
                              {job.selection_process && (<div className="space-y-2">
                              ...
                              {job.apply_link && (<a href={job.apply_link} target="_blank" rel="noopener noreferrer" className="block">
                        ...
                        {job.apply_link && (<a href={job.apply_link} target="_blank" rel="noopener noreferrer" className="flex-1">
        ...
        <Card>
          ...
          <CardContent>
            {mentorshipsLoading ? (<div className="flex justify-center py-8">
                <Loader2 className="w-6 h-6 animate-spin text-gold"/>
              </div>) : mentorships && mentorships.length > 0 ? (<div className="grid gap-4 md:grid-cols-2">
                {mentorships.map((mentor, index) => (<motion.div key={mentor.id} initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: index * 0.1 }} className="bg-muted/50 rounded-xl p-6 border border-transparent hover:border-gold/20 transition-all">
                    <div className="flex items-start gap-4">
                      <div className="w-14 h-14 rounded-full bg-gold/10 flex items-center justify-center border-2 border-background shadow-sm shrink-0 overflow-hidden">
                        {mentor.alumni?.photo_url ? (<img src={mentor.alumni.photo_url} className="w-full h-full object-cover"/>) : (<span className="text-gold font-bold text-xl">
                            {mentor.alumni?.full_name?.charAt(0) || "M"}
                          </span>)}
                      </div>
                      <div className="flex-1">
                        <h3 className="font-semibold text-lg text-foreground">{mentor.title}</h3>
                        {mentor.alumni && (<p className="text-gold text-sm font-medium">
                        ...
                        {mentor.expertise_areas && mentor.expertise_areas.length > 0 && (<div className="flex flex-wrap gap-1 mt-4">
                            {mentor.expertise_areas.map((area) => (<Badge key={area} variant="secondary" className="text-[10px] bg-gold/5 text-gold hover:bg-gold/10 border-gold/10">
                                {area}
                              </Badge>))}
                          </div>)}
                        
                        {(mentor.contact_email || mentor.contact_phone) && (<div className="mt-4 pt-4 border-t border-border/50 flex flex-wrap gap-4 items-center">
                            {mentor.contact_email && (<a href={`mailto:${mentor.contact_email}`} className="text-xs text-gold hover:underline flex items-center gap-1">
                                <ExternalLink className="w-3 h-3"/>
                                {mentor.contact_email}
                              </a>)}
                            {mentor.contact_phone && (<p className="text-xs text-muted-foreground">{mentor.contact_phone}</p>)}
                          </div>)}
                      </div>
                    </div>
                  </motion.div>))}
      ...
      {showNetworkTabs && (<TabsContent value="connections">
          <ConnectionPage profile={profile}/>
        </TabsContent>)}

      {showNetworkTabs && (<TabsContent value="chat">
