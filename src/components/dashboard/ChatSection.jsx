...
const ChatSection = ({ profile, unreadBadge = 0 }) => {
    ...
    const activeConnection = useMemo(() => connections.find((item) => item.id === selectedConnectionId) || null, [connections, selectedConnectionId]);
    ...
    return (<Card>
      <CardHeader>
        <CardTitle className="font-serif flex items-center gap-2">
          ...
          Chat
          {unreadBadge > 0 && (<Badge variant="destructive" className="ml-auto">
              {unreadBadge}
            </Badge>)}
        </CardTitle>
      </CardHeader>
      <CardContent>
        {connectionsLoading ? (<div className="flex justify-center py-8">
            <Loader2 className="w-6 h-6 animate-spin text-gold"/>
          </div>) : connections.length === 0 ? (<p className="text-muted-foreground text-center py-10">
            No active connections yet. Accept a connection request to start chatting.
          </p>) : (<div className="grid md:grid-cols-[260px_1fr] gap-4">
            <div className="space-y-2">
              {connections.map((connection) => {
                const hasUnread = unreadCounts[connection.id] > 0;
                return (<button key={connection.id} className={`w-full text-left rounded-lg border px-3 py-2 transition-all ${selectedConnectionId === connection.id
                        ? "bg-gold/10 border-gold/40"
                        : hasUnread
                            ? "bg-destructive/5 border-destructive/30 hover:bg-destructive/10"
                            : "bg-background hover:bg-muted/40"}`} onClick={() => setSelectedConnectionId(connection.id)}>
                    <div className="flex items-center justify-between">
                      ...
                      {hasUnread && (<Badge variant="destructive" className="ml-2 shrink-0 text-xs">
            ...
            <div className="border rounded-xl p-3 flex flex-col h-[460px]">
              ...
              <div className="pt-3 border-t mt-3 flex gap-2">
                <Input value={draft} onChange={(event) => setDraft(event.target.value)} placeholder="Type your message" disabled={!selectedConnectionId || sendMessage.isPending} onKeyDown={(event) => {
                if (event.key === "Enter") {
