...
const Messages = () => {
    ...
    if (error)
        return <p className="text-center py-10 text-red-500">Error loading messages</p>;
    if (!data || data.length === 0)
        return null;
    const marqueeData = [...data, ...data];
    return (<div className="w-full overflow-hidden py-10 bg-gray-50">
      <div className="flex w-max animate-marquee gap-6 hover:[animation-play-state:paused]">
        {marqueeData.map((alumni, index) => (<div key={index} className="w-[360px] flex-shrink-0 rounded-2xl border border-gray-100 bg-white p-6 shadow-sm transition-shadow hover:shadow-md">
            <div className="flex items-center gap-4">
              
              {/* AVATAR WITH SMALL PROFILE PIC */}
              <div className="relative">
                {alumni.photo_url ? (<img src={alumni.photo_url} alt={alumni.name} className="h-14 w-14 rounded-full object-cover ring-2 ring-gray-50"/>) : (<div className="h-14 w-14 rounded-full bg-gray-200 flex items-center justify-center font-bold text-gray-400">
