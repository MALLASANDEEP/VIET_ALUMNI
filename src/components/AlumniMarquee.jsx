    ...
    <div className="relative h-64 w-full overflow-hidden bg-slate-900">
      ...
      <div className="absolute bottom-4 left-6 z-20">
        ...
        <div className="flex items-center gap-1.5">
          ...
          {alumni.is_verified && (<BadgeCheck className="w-5 h-5 text-blue-400 shrink-0"/>)}
        </div>
      </div>
...
const AlumniMarquee = () => {
    ...
    return (<section className="pt-6 pb-20 bg-[#f8fafc] overflow-hidden relative">
      ...
      <div className="relative flex gap-4">
        <div className="flex animate-marquee hover:[animation-play-state:paused]">
          {[...alumniData, ...alumniData].map((alumni, index) => (<AlumniCard key={`marquee-${alumni.id}-${index}`} alumni={alumni} onClick={setSelectedAlumni}/>))}
        </div>
      </div>

      <AnimatePresence>
        {selectedAlumni && (<motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }} exit={{ opacity: 0 }} className="fixed inset-0 bg-slate-950/90 backdrop-blur-md z-[100] flex items-center justify-center p-6" onClick={() => setSelectedAlumni(null)}>
            <motion.div initial={{ scale: 0.9, opacity: 0, y: 20 }} animate={{ scale: 1, opacity: 1, y: 0 }} exit={{ scale: 0.9, opacity: 0, y: 20 }} className="bg-white rounded-[2.5rem] max-w-md w-full relative overflow-hidden shadow-2xl" onClick={(e) => e.stopPropagation()}>
              {/* Top Banner with Logo */}
              <div className="h-32 bg-orange-500 flex items-center justify-center p-6 relative">
                 <button onClick={() => setSelectedAlumni(null)} className="absolute top-4 right-4 z-50 p-1.5 rounded-full bg-black/10 hover:bg-black/20 text-white transition-all">
                    <X className="w-5 h-5"/>
                  </button>
                  <img src="https://www.viet.edu.in/img/header-imgs/viet-logo.svg" alt="College Logo" className="h-10 opacity-90"/>
              </div>
              ...
              <div className="px-8 -mt-12 relative z-10 flex items-end justify-between gap-4">
                ...
                <div className="flex-shrink-0">
                  <img src={selectedAlumni.photo_url || ""} alt={selectedAlumni.name} className="w-28 h-36 object-cover rounded-2xl border-4 border-white shadow-xl bg-slate-100"/>
                </div>
              </div>

              {/* Information Section */}
              <div className="p-8 pt-6 space-y-6">
                {(selectedAlumni.current_position || selectedAlumni.company) && (<div className="flex items-start gap-4 p-4 bg-slate-50 rounded-2xl border border-slate-100">
                ...
                {selectedAlumni.message && (<div className="bg-indigo-50/50 p-4 rounded-2xl border-l-4 border-indigo-600">
                    <p className="text-sm text-slate-600 italic leading-relaxed">
                      "{selectedAlumni.message}"
                    </p>
                  </div>)}

                {selectedAlumni.linkedin && (<a href={selectedAlumni.linkedin} target="_blank" rel="noopener noreferrer" className="w-full py-3.5 bg-indigo-600 hover:bg-indigo-700 text-white rounded-xl flex items-center justify-center gap-2 text-sm font-bold transition-all shadow-lg shadow-indigo-100">
