...
const GalleryHero = () => {
    ...
    const backgroundImage = homeHero.bg_type === "image" && homeHero.bg_images?.length > 0
        ...
        : "/default-hero.jpg";
    return (<section className="relative h-[70vh] flex items-end text-white overflow-hidden">
      {/* Sliding Background Image */}
      <img src={backgroundImage} className="absolute inset-0 w-full h-full object-cover transition-opacity duration-1000" alt="Gallery hero background"/>
      ...
      <div className="relative z-10 max-w-2xl mb-16 ml-8">
        {heroData?.title && (<h1 className="text-4xl md:text-6xl font-bold">{heroData.title}</h1>)}
        {heroData?.subtitle && (<p className="mt-4 text-lg opacity-90">{heroData.subtitle}</p>)}
      </div>
    </section>);
};
