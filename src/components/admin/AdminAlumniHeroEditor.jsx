...
const AdminAlumniHeroEditor = () => {
    ...
    return (<div className="max-w-xl mx-auto p-4 sm:p-6 bg-white rounded-xl shadow-md space-y-4">
      ...
      <div className="flex flex-col space-y-2">
        ...
        <Input value={hero.title || ""} onChange={(e) => setHero({ ...hero, title: e.target.value })} placeholder="Enter hero title"/>
      </div>

      {/* Subtitle */}
      <div className="flex flex-col space-y-2">
        <label className="text-sm font-semibold">Hero Subtitle</label>
        <Input value={hero.subtitle || ""} onChange={(e) => setHero({ ...hero, subtitle: e.target.value })} placeholder="Enter hero subtitle"/>
      </div>

      {/* Apply Button Text */}
      <div className="flex flex-col space-y-2">
        <label className="text-sm font-semibold">Apply Button Text</label>
        <Input value={hero.apply_text || ""} onChange={(e) => setHero({ ...hero, apply_text: e.target.value })} placeholder="Apply Now"/>
      </div>

      {/* Apply Button URL */}
      <div className="flex flex-col space-y-2">
        <label className="text-sm font-semibold">Apply Button URL</label>
        <Input value={hero.apply_url || ""} onChange={(e) => setHero({ ...hero, apply_url: e.target.value })} placeholder="https://example.com/apply"/>
      </div>

      {/* Background Type */}
      <div className="flex flex-col space-y-2">
        <label className="text-sm font-semibold">Background Type</label>
        <select value={hero.bg_type || "image"} onChange={(e) => setHero({ ...hero, bg_type: e.target.value })} className="h-11 px-4 rounded-xl border border-slate-200 text-sm">
