...
const AdminManageAdmins = () => {
    ...
    return (<div className="min-h-screen bg-gray-100 p-4 sm:p-6 lg:p-8">
      <div className="max-w-4xl mx-auto bg-white shadow-lg rounded-xl p-4 sm:p-6">
        ...
        <form onSubmit={handleAddAdmin} className="space-y-4 mb-8">
          <div>
            ...
            <input type="text" value={fullName} onChange={(e) => setFullName(e.target.value)} className="w-full border p-2 rounded" placeholder="Enter full name"/>
          </div>

          <div>
            <label className="block font-medium">Email</label>
            <input type="email" value={email} onChange={(e) => setEmail(e.target.value)} className="w-full border p-2 rounded" placeholder="Enter email" required/>
          </div>
        ...
        {fetching ? (<p>Loading admins...</p>) : admins.length === 0 ? (<p>No admins found.</p>) : (<div className="space-y-3">
            {admins.map((admin) => (<div key={admin.user_id} className="flex flex-col sm:flex-row sm:justify-between sm:items-center gap-3 border p-3 rounded">
                <div>
                ...
                <button onClick={() => handleDelete(admin.user_id)} className="bg-red-600 text-white px-4 py-1 rounded hover:bg-red-700 transition w-full sm:w-auto">
                  Remove
                </button>
              </div>))}
          </div>)}
