...
const ContentGalleryAdmin = () => {
    ...
    return (<div className="max-w-5xl mx-auto px-1 sm:px-0 py-6 sm:py-10 space-y-6 sm:space-y-8">
      ...
      <div className="space-y-4">
        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 gap-4">
          {Array.from({ length: MAX_IMAGES }).map((_, index) => (<div key={index} className="space-y-2">
              ...
              {uploadingIndex === index && (<p className="text-sm text-gray-500">Uploading...</p>)}

              {images[index] && (<div className="relative">
                  <img src={images[index]} alt={`Preview ${index + 1}`} className="h-32 w-full object-cover rounded"/>

                  <button type="button" onClick={() => handleImageDelete(index)} className="absolute top-2 right-2 bg-red-600 text-white p-1 rounded">
                    <Trash size={14}/>
                  </button>
                </div>)}
            </div>))}
      ...
      <Button onClick={handleSave} disabled={updateContent.isPending} className="flex gap-2 w-full sm:w-auto">
        <Save size={18}/>
        Save Changes
      </Button>
    </div>);
