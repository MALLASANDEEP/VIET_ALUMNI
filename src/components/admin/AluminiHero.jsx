...
import { UserPlus, Trash2, Loader2, X, Settings2, Edit, Paperclip, } from "lucide-react";
import { Button } from "@/components/ui/button";
import { Input } from "@/components/ui/input";
import { Card, CardContent, CardHeader, CardTitle } from "@/components/ui/card";
import { useAlumni, useAddAlumni, useUpdateAlumni, useDeleteAlumni, useUpdateAlumniTitle, uploadAlumniPhoto, } from "@/hooks/useAlumni";
...
export const AdminAlumnihero = () => {
    ...
    const handleSubmit = async (e) => {
        e.preventDefault();
        if (!formData.name || !formData.batch)
            return;
        const payload = {
            ...formData,
    ...
    const handleEdit = (alumni) => {
        ...
        setAttachmentUrl(alumni.attachment_url || null);
        setShowForm(true);
    };
    return (<div className="space-y-8">
      ...
      <Card>
        ...
        <CardContent className="flex gap-4">
          <Input placeholder={currentTitle} value={newSectionTitle} onChange={(e) => setNewSectionTitle(e.target.value)}/>
      ...
      <Card>
        ...
        <CardContent>
          <AnimatePresence>
            {showForm && (<motion.form onSubmit={handleSubmit} initial={{ opacity: 0 }} animate={{ opacity: 1 }} exit={{ opacity: 0 }} className="space-y-4 mb-6">
                <Input placeholder="Name" value={formData.name} onChange={(e) => setFormData({ ...formData, name: e.target.value })}/>
                <Input placeholder="Batch" value={formData.batch} onChange={(e) => setFormData({ ...formData, batch: e.target.value })}/>
                ...
                <input type="file" hidden ref={attachmentInputRef} onChange={handleAttachmentUpload}/>

                {attachmentUrl && (<p className="text-xs text-indigo-600">
          ...
          {isLoading ? (<Loader2 className="animate-spin"/>) : (alumniList.map((alumni) => (<Card key={alumni.id}>
                <CardContent className="flex justify-between items-center">
                  <div className="flex gap-3">
                    ...
                    <div>
                      ...
                      {alumni.attachment_url && (<a href={alumni.attachment_url} target="_blank" className="text-xs text-indigo-600 flex items-center gap-1">
                  ...
                  <div className="flex gap-2">
                    ...
                    <Button size="sm" variant="destructive" onClick={() => handleDelete(alumni.id)}>
