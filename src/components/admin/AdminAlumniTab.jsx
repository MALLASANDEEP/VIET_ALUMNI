    ...
    return (<div className="space-y-8">
      ...
      <Card className="border-border/50 shadow-xl">
        ...
        <CardContent className="pt-6">
          <AnimatePresence>
            {showForm && (<motion.form initial={{ opacity: 0, y: -20 }} animate={{ opacity: 1, y: 0 }} exit={{ opacity: 0, y: -20 }} onSubmit={handleSubmit} className="bg-muted/50 rounded-2xl p-6 mb-8 border border-indigo-100 space-y-4">
                {/* FORM GRID */}
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
            {isLoading ? (<div className="col-span-full flex justify-center py-20">
                <Loader2 className="w-8 h-8 animate-spin text-indigo-600"/>                                         
              </div>) : (alumniList.map(alumni => (<Card key={alumni.id} className="border-border/50 shadow">
