...
import { GraduationCap, Loader2, ArrowLeft, UserCheck, Users, Camera, Upload } from "lucide-react";
...
const Register = () => {
    ...
    const handleSubmit = async (e) => {
        ...
        try {
            ...
            const { error: profileError } = await supabase
                ...
                lpa: formData.lpa ? parseFloat(formData.lpa) : null,
    ...
    return (<div className="min-h-screen bg-gradient-to-br from-navy-dark via-navy to-navy-dark flex items-center justify-center p-4">
          ...
          <CardContent>
            <form onSubmit={handleSubmit} className="space-y-6">
              ...
              <div className="grid grid-cols-2 gap-4">
                <button type="button" onClick={() => setFormData(prev => ({ ...prev, requestedRole: "student" }))} className={`p-4 rounded-xl border-2 transition-all flex flex-col items-center ${formData.requestedRole === "student"
            ? "border-gold bg-gold/10"
            : "border-border hover:border-gold/30"}`}>
                  <Users className={`w-8 h-8 mb-2 ${formData.requestedRole === "student"
            ? "text-gold"
            : "text-muted-foreground"}`}/>
                  <span className="font-semibold text-sm">Student</span>
                </button>

                <button type="button" onClick={() => setFormData(prev => ({ ...prev, requestedRole: "alumni" }))} className={`p-4 rounded-xl border-2 transition-all flex flex-col items-center ${formData.requestedRole === "alumni"
            ? "border-gold bg-gold/10"
            : "border-border hover:border-gold/30"}`}>
                  <UserCheck className={`w-8 h-8 mb-2 ${formData.requestedRole === "alumni"
            ? "text-gold"
            : "text-muted-foreground"}`}/>
              ...
              <div className="flex flex-col items-center gap-3">
                ...
                <div onClick={() => fileInputRef.current?.click()} className="relative w-20 h-20 rounded-full bg-muted border-2 border-dashed border-gold/40 flex items-center justify-center cursor-pointer hover:bg-gold/5 overflow-hidden transition-all">
                  {formData.photo ? (<img src={URL.createObjectURL(formData.photo)} alt="Preview" className="w-full h-full object-cover"/>) : (<Camera className="w-6 h-6 text-muted-foreground"/>)}
              ...
              <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                <div className="space-y-2">
                  ...
                  <Select onValueChange={(v) => setFormData({ ...formData, department: v })}>
                    ...
                    <SelectContent>
                      {departments.map((d) => (<SelectItem key={d} value={d}>
                          {d}
                        </SelectItem>))}
                    </SelectContent>
                  </Select>
              ...
              {/* ALUMNI FIELDS */}
              {formData.requestedRole === "alumni" && (<motion.div initial={{ opacity: 0, height: 0 }} animate={{ opacity: 1, height: "auto" }} className="space-y-4 pt-4 border-t border-gold/10">
                  <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                    <Input placeholder="Company" value={formData.company} onChange={(e) => setFormData({ ...formData, company: e.target.value })}/>
                    <Input placeholder="Position" value={formData.currentPosition} onChange={(e) => setFormData({
                ...formData,
                currentPosition: e.target.value,
            })}/>
                  </div>

                  <Textarea placeholder="Professional Bio" value={formData.bio} onChange={(e) => setFormData({ ...formData, bio: e.target.value })}/>

                  <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                    <Input type="number" placeholder="LPA" value={formData.lpa} onChange={(e) => setFormData({ ...formData, lpa: e.target.value })}/>
                    <Input type="url" placeholder="LinkedIn URL" value={formData.linkedin} onChange={(e) => setFormData({ ...formData, linkedin: e.target.value })}/>
                  </div>
                </motion.div>)}
