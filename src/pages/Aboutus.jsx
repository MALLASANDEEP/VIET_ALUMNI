...
const AboutSection = ({ title, content, image, reverse = false, // ✅ default value
link, }) => {
    return (<section className="py-6 overflow-hidden">
      <div className="grid md:grid-cols-2 items-center md:gap-4 max-w-8xl mx-auto">
        {/* IMAGE */}
        <div className={`${reverse ? "md:order-2 md:ml-auto" : "md:order-1 md:mr-auto"} w-screen md:w-full`}>
          {image && (<img src={image} alt={title} className={`h-[380px] w-full object-cover ${reverse ? "rounded-l-[140px]" : "rounded-r-[140px]"}`}/>)}
        </div>

        {/* CONTENT */}
        <div className={`px-4 md:px-6 max-w-xl mx-auto ${reverse ? "md:order-1" : "md:order-2"}`}>
          <h2 className="text-3xl md:text-4xl font-bold mb-4">{title}</h2>
          <p className="text-gray-700 leading-relaxed text-base md:text-lg mb-4">
            {content}
          </p>
          {link && (<a href={link} target="_blank" rel="noopener noreferrer" className="bg-blue-600 text-white px-6 py-2 rounded-full font-semibold hover:bg-blue-700 transition">
              Know More →
            </a>)}
        </div>
      </div>
      ...
      <section className="relative text-white h-[85vh] flex items-end overflow-hidden">
        ...
        <div className="absolute inset-0 -z-10">
          <AnimatePresence mode="wait">
            <motion.div key={backgroundImage} initial={{ opacity: 0 }} animate={{ opacity: 1 }} exit={{ opacity: 0 }} transition={{ duration: 1 }} className="absolute inset-0 bg-cover bg-center" style={{ backgroundImage: `url(${backgroundImage})` }}/>
          </AnimatePresence>
        </div>
      ...
      <section className="py-6 text-center bg-gradient-to-r from-primary/20 via-gold/20 to-primary/20 relative">
        <div className="max-w-4xl mx-auto p-12 bg-black/20 backdrop-blur-md rounded-3xl shadow-xl">
          ...
          <div className="flex justify-center gap-6 flex-wrap">
            <a href="https://docs.google.com/forms/d/e/1FAIpQLSe8Ca17CXUJLTO2ZYkfLUveic3f8DUo1cMEtLYToUE_Iu6TOw/viewform?pli=1" target="_blank" rel="noopener noreferrer" className="bg-white text-blue-600 px-8 py-3 rounded-full font-semibold hover:bg-gray-200 transition">
              Apply Now
            </a>
          </div>
        </div>
