...
import { Shield, Users, Image, LogOut, GraduationCap, LayoutDashboard, Loader2, UserCheck, Trash2, } from "lucide-react";
      ...
      <header className="bg-none sticky top-0 z-50">
        <div className="container mx-auto px-4 sm:px-6 py-3 sm:h-16 flex flex-col sm:flex-row justify-between sm:items-center gap-3">
          ...
          <div className="flex flex-wrap gap-2 w-full sm:w-auto">
            ...
            <Button variant="ghost" onClick={signOut} className="bg-black text-white hover:bg-black/90 w-full sm:w-auto">
            ...
            <TabsContent value="settings" className="mt-0">
              <Card>
                ...
                <CardContent className="space-y-6">
                  {!heroDraft ? (<p>Loading hero...</p>) : (<>
                      <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                        <Input value={heroDraft.badge_text || ""} onChange={(e) => setHeroDraft({ ...heroDraft, badge_text: e.target.value })}/>
                        <Input value={heroDraft.title || ""} onChange={(e) => setHeroDraft({ ...heroDraft, title: e.target.value })}/>
                      </div>

                      <Input value={heroDraft.subtitle || ""} onChange={(e) => setHeroDraft({ ...heroDraft, subtitle: e.target.value })}/>
                ...
                setHeroDraft({
                    ...
                    bg_images: [...(heroDraft.bg_images || []), ...urls],
                      ...
                      {heroDraft.bg_images?.length > 0 && (<div className="grid grid-cols-2 sm:grid-cols-3 gap-3">
                          {heroDraft.bg_images.map((img, i) => (<div key={i} className="relative">
                              <img src={img} className="h-24 w-full object-cover rounded"/>
                              <button className="absolute top-1 right-1 bg-black/70 p-1 rounded" onClick={() => setHeroDraft({
                        ...heroDraft,
                        bg_images: heroDraft.bg_images.filter((_, idx) => idx !== i),
                    })}>
