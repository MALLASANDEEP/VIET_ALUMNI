...
const ContentGalleryUI = () => {
    ...
    return (<section className="bg-[#FDFDFD] py-8 w-full px-4">
      <div className="max-w-6xl mx-auto grid grid-cols-1 md:grid-cols-12 bg-white rounded-[32px] overflow-hidden shadow-[0_25px_60px_-15px_rgba(0,0,0,0.06)] border border-gray-100">
        ...
        <div className="md:col-span-5 relative h-[350px] md:h-[480px] flex items-center justify-center bg-[#fafafa] overflow-hidden">
          ...
          <button onClick={prev} className="absolute left-4 top-1/2 -translate-y-1/2 z-30 w-10 h-10 rounded-full bg-white/90 backdrop-blur-md shadow-md flex items-center justify-center hover:scale-110 active:scale-95 transition-all">
          ...
          <div className="relative w-full h-full flex items-center justify-center">
            ...
            if (!isActive && !isPrev && !isNext)
                return null;
            return (<motion.img key={src} src={src} initial={{ opacity: 0, scale: 0.8 }} animate={{
            ...
            className="absolute h-[92%] w-[85%] object-cover rounded-[24px] shadow-lg" alt={`Gallery ${i}`}/>);
        })}
            </AnimatePresence>
          </div>

          <button onClick={next} className="absolute right-4 top-1/2 -translate-y-1/2 z-30 w-10 h-10 rounded-full bg-white/90 backdrop-blur-md shadow-md flex items-center justify-center hover:scale-110 active:scale-95 transition-all">
        ...
        <div className="md:col-span-7 flex flex-col justify-center px-8 md:px-14 py-10 md:py-14">
          ...
          <a href={content.button_link} className="inline-flex items-center gap-3 text-xs font-bold uppercase tracking-widest group w-fit">
            {content.button_text}
          ...
          <div className="mt-10 flex gap-2.5">
            {images.map((_, i) => (<button key={i} onClick={() => setIndex(i)} className={`h-1.5 rounded-full transition-all duration-500 ${i === index
                ? "w-10 bg-slate-900"
                : "w-1.5 bg-slate-200 hover:bg-slate-300"}`} aria-label={`Slide ${i + 1}`}/>))}
          </div>
        </div>
      </div>
    </section>);
