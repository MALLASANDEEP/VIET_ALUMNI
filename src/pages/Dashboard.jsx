...
import { Loader2, GraduationCap, Users, Clock, CheckCircle, XCircle, Home, Ban } from "lucide-react";
...
const Dashboard = () => {
    ...
    if (isAdmin) {
        const previewProfile = allProfiles.find((p) => p.status === "approved" && p.requested_role === previewRole);
        ...
        <div className="container mx-auto px-4 sm:px-6 py-6 sm:py-8 space-y-6">
          <div className="flex flex-wrap gap-2">
            <Button variant={previewRole === "student" ? "default" : "outline"} onClick={() => setPreviewRole("student")}>
              Student View
            </Button>
            <Button variant={previewRole === "alumni" ? "default" : "outline"} onClick={() => setPreviewRole("alumni")}>
              Alumni View
            </Button>
          </div>
          ...
          {!previewProfile ? (<Card>
            ...
            </Card>) : previewRole === "student" ? (<StudentDashboard profile={previewProfile} showNetworkTabs={false}/>) : (<AlumniDashboard profile={previewProfile}/>)}
        </div>
      </div>);
    }
    // Show banned message
    ...
    return (<div className="min-h-screen bg-background">
      ...
      <div className="container mx-auto px-6 py-8">
        ...
        {/* Dashboard Content */}
        {profile.requested_role === "student" ? (<StudentDashboard profile={profile}/>) : (<AlumniDashboard profile={profile}/>)}
      </div>
    </div>);
};
